...
int main()
{
  ...
  camera_t* camera = camera_open("/dev/video0", 176, 144);
  printf("Init'ing camera...\n");
  camera_init(camera);
  ...
  unsigned char* rgb = camera->head.start;
  FILE* out = fopen("result.jpg", "w");
  FILE* out2 = fopen("result.raw", "w");
  jpeg(out, rgb, camera->width, camera->height, 100);
  fwrite(rgb,sizeof(unsigned char), camera->width*camera->height, out2); 
  fclose(out);
  fclose(out2);
  //free(rgb);
